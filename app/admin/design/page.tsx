...
import { summarizeVersionChanges, type VersionChange } from "@/lib/publishing";
import type {
  HomepageDesignDraft,
...
import { resetGlobalDesignDraft, restoreGlobalDesignVersion } from "./global-actions";
import {
...
async function HomeEditor() {
  ...
  return <>
    <div className="visualEditorIntro"><div><span className={`visualStatus ${draft ? "draft" : "published"}`}>{draft ? "Есть неопубликованный черновик" : "Опубликованный дизайн"}</span><p>Главная поддерживает слои Hero, Canvas 2.0 и отдельно управляет порядком и видимостью секций.</p></div>{draft && <form action={resetVisualDraft}><button type="submit" className="rowAction danger">Сбросить черновик</button></form>}</div>
...
async function GlobalEditor({ componentKey }: { componentKey: "header" | "footer" }) {
  const { supabase } = await requireEditor();
  const [rowResult, draftResult, versionsResult, assetsResult] = await Promise.all([
    supabase.from("site_global_designs").select("*").eq("component_key", componentKey).maybeSingle(),
    supabase.from("site_global_design_drafts").select("*").eq("component_key", componentKey).maybeSingle(),
    supabase.from("site_global_design_versions").select("*").eq("component_key", componentKey).order("created_at", { ascending: false }).limit(20),
    supabase.from("design_media_assets").select("*").eq("is_active", true).order("created_at", { ascending: false }).limit(60),
  ]);
  ...
  const published = componentKey === "header" ? normalizeHeaderDesign(row?.config, defaultHeaderDesign) : normalizeFooterDesign(row?.config, defaultFooterDesign);
  const initial = componentKey === "header" ? normalizeHeaderDesign(draftRow?.config, published as any) : normalizeFooterDesign(draftRow?.config, published as any);
  const versions = (versionsResult.data ?? []) as { id: string | number; label: string | null; created_at: string; snapshot: unknown; change_summary?: VersionChange[] }[];
  const assets = (assetsResult.data ?? []) as DesignMediaAsset[];
  const title = componentKey === "header" ? "Global Header" : "Global Footer";
  return <>
    <div className="visualEditorIntro"><div><span className={`visualStatus ${hasDraft ? "draft" : "published"}`}>{hasDraft ? `Черновик: ${title}` : `Опубликовано: ${title}`}</span><p>Глобальный компонент применяется ко всем публичным страницам сайта после публикации.</p></div>{hasDraft && <form action={resetGlobalDesignDraft}><input type="hidden" name="component_key" value={componentKey}/><button type="submit" className="rowAction danger">Сбросить черновик</button></form>}</div>
    <GlobalDesignEditor componentKey={componentKey} initial={initial} assets={assets} />
    <section className="visualHistory"><div className="sectionHeading"><div><p className="eyebrow blue">ИСТОРИЯ</p><h2>Версии {componentKey === "header" ? "Header" : "Footer"}</h2><p>Publishing 2.0 показывает изменения каждой публикации. Восстановление сначала помещает старую версию в черновик.</p></div></div>{versions.length ? <div className="visualHistoryList">{versions.map((version, index) => <article className="visualHistoryRow versionHistory2" key={version.id}><div className="versionHistoryHead"><div><strong>{version.label || `Версия #${version.id}`}</strong><span>{formatDate(version.created_at)}</span></div><form action={restoreGlobalDesignVersion}><input type="hidden" name="component_key" value={componentKey}/><input type="hidden" name="version_id" value={String(version.id)}/><button className="rowAction" type="submit">Восстановить в черновик</button></form></div><VersionCompare changes={versionChanges(version, versions[index + 1])} /></article>)}</div> : <div className="adminEmpty">История появится после первой публикации.</div>}</section>
  </>;
}
...
async function DesignSystemGlobalEditor() {
  ...
  return <>
    ...
    <section className="visualHistory"><div className="sectionHeading"><div><p className="eyebrow blue">ИСТОРИЯ</p><h2>Версии Design System</h2><p>Сравнение показывает изменённые design tokens относительно предыдущей публикации.</p></div></div>{versions.length ? <div className="visualHistoryList">{versions.map((version, index) => <article className="visualHistoryRow versionHistory2" key={version.id}><div className="versionHistoryHead"><div><strong>{version.label || `Версия #${version.id}`}</strong><span>{formatDate(version.created_at)}</span></div><form action={restoreGlobalDesignVersion}><input type="hidden" name="component_key" value="design_system"/><input type="hidden" name="version_id" value={String(version.id)}/><button className="rowAction" type="submit">Восстановить в черновик</button></form></div><VersionCompare changes={versionChanges(version, versions[index + 1])} /></article>)}</div> : <div className="adminEmpty">История появится после первой публикации.</div>}</section>
  </>;
}
...
function HomeHistory({ versions }: { versions: HomepageDesignVersion[] }) {
  return <section className="visualHistory"><div className="sectionHeading"><div><p className="eyebrow blue">ИСТОРИЯ</p><h2>Версии главной</h2><p>Publishing 2.0 хранит краткий список изменений и позволяет сравнить публикации.</p></div></div>{versions.length ? <div className="visualHistoryList">{versions.map((version, index) => <article className="visualHistoryRow versionHistory2" key={version.id}><div className="versionHistoryHead"><div><strong>{version.label || `Версия #${version.id}`}</strong><span>{formatDate(version.created_at)}</span></div><div className="visualHistoryMeta"><span>{version.snapshot?.hero_height_desktop ?? 650}px</span></div><form action={restoreVisualVersion}><input type="hidden" name="version_id" value={String(version.id)} /><button className="rowAction" type="submit">Восстановить в черновик</button></form></div><VersionCompare changes={versionChanges(version, versions[index + 1])} /></article>)}</div> : <div className="adminEmpty">История появится после первой публикации.</div>}</section>;
}

function SitePageHistory({ pageKey, versions }: { pageKey: SitePageDesignKey; versions: SitePageDesignVersion[] }) {
  return <section className="visualHistory"><div className="sectionHeading"><div><p className="eyebrow blue">ИСТОРИЯ</p><h2>Версии выбранной страницы</h2><p>Каждая страница и шаблон имеют независимую историю и сравнение изменений.</p></div></div>{versions.length ? <div className="visualHistoryList">{versions.map((version, index) => <article className="visualHistoryRow versionHistory2" key={version.id}><div className="versionHistoryHead"><div><strong>{version.label || `Версия #${version.id}`}</strong><span>{formatDate(version.created_at)}</span></div><div className="visualHistoryMeta"><span>{version.snapshot?.hero_height_desktop ?? "—"}px</span><span>{version.snapshot?.background_mode ?? "default"}</span></div><form action={restoreSitePageVisualVersion}><input type="hidden" name="page_key" value={pageKey}/><input type="hidden" name="version_id" value={String(version.id)}/><button className="rowAction" type="submit">Восстановить в черновик</button></form></div><VersionCompare changes={versionChanges(version, versions[index + 1])} /></article>)}</div> : <div className="adminEmpty">История появится после первой публикации этой страницы.</div>}</section>;
}

function VersionCompare({ changes }: { changes: VersionChange[] }) {
  if (!changes.length) return <div className="versionNoChanges">Исходная версия или изменения не были записаны старой версией редактора.</div>;
  return <details className="versionCompare"><summary>Сравнить изменения <span>{changes.length}</span></summary><div className="versionDiffList">{changes.slice(0, 24).map((change) => <div className="versionDiffRow" key={`${change.path}-${change.before}-${change.after}`}><strong>{change.label}</strong><span className="before">{change.before}</span><b>→</b><span className="after">{change.after}</span></div>)}</div>{changes.length > 24 && <small>Показаны первые 24 изменения.</small>}</details>;
}

function versionChanges(current: { snapshot: unknown; change_summary?: VersionChange[] }, previous?: { snapshot: unknown }) {
  if (Array.isArray(current.change_summary) && current.change_summary.length) return current.change_summary;
  return previous ? summarizeVersionChanges(previous.snapshot, current.snapshot) : [];
}
