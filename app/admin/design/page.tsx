      ...
      <section className="adminHero compactAdminHero visualAdminHero">
        <div className="container adminHeroInner">
          <div>
            ...
            <p>Hero и Header-зоны всех основных страниц: Desktop/Mobile, фото, crop, zoom, overlay, текст, черновики и история.</p>
          </div>
...
async function GenericEditor({ pageKey }: { pageKey: SitePageDesignKey }) {
  ...
  const [publishedResult, draftResult, versionsResult] = await Promise.all([
  ...
  return <>
    ...
    <SitePageVisualEditor pageKey={pageKey} item={item} initial={initial} hasDraft={Boolean(draft)} />
...
function buildPublished(hero: HomepageHero | null, sections: HomepageSection[]): HomepageDesignSnapshot {
  ...
  return { background_image_url: hero?.background_image_url ?? null, mobile_background_image_url: hero?.mobile_background_image_url ?? null, desktop_position_x: hero?.desktop_position_x ?? position.x, desktop_position_y: hero?.desktop_position_y ?? position.y, desktop_zoom_percent: hero?.desktop_zoom_percent ?? 100, mobile_position_x: hero?.mobile_position_x ?? position.x, mobile_position_y: hero?.mobile_position_y ?? position.y, mobile_zoom_percent: hero?.mobile_zoom_percent ?? 100, hero_height_desktop: hero?.hero_height_desktop ?? 650, hero_height_mobile: hero?.hero_height_mobile ?? 520, overlay_opacity: hero?.overlay_opacity ?? 72, text_alignment: hero?.text_alignment ?? "left", show_match_card: hero?.show_match_card ?? true, section_order: order, section_visibility: visibility };
}
