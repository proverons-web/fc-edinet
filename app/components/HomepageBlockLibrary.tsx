        ...
        <div className="blockLibraryInspector">
          {!selected ? <div className="adminEmpty">Добавь или выбери пользовательский блок.</div> : <>
            ...
            <div className="blockInspectorGroup">
              ...
              <div className="sectionBuilderFieldGrid"><label><span>Отступ сверху</span><input type="number" min={0} max={180} value={selected.design.padding_top} onFocus={(e)=>e.currentTarget.select()} onChange={(e)=>{const value=e.currentTarget.valueAsNumber;if(Number.isFinite(value))updateDesign({padding_top:clamp(value,0,180)});}}/></label><label><span>Отступ снизу</span><input type="number" min={0} max={180} value={selected.design.padding_bottom} onFocus={(e)=>e.currentTarget.select()} onChange={(e)=>{const value=e.currentTarget.valueAsNumber;if(Number.isFinite(value))updateDesign({padding_bottom:clamp(value,0,180)});}}/></label></div>
              {blockSupportsItemLimit(selected.type) && <label className="blockSingleField"><span>{selected.type === "standings" ? "Строк таблицы" : "Количество элементов"}</span><input type="number" min={1} max={selected.type === "partners" ? 24 : 12} value={selected.design.item_limit} onFocus={(e)=>e.currentTarget.select()} onChange={(e)=>{const value=e.currentTarget.valueAsNumber;if(Number.isFinite(value))updateDesign({item_limit:clamp(value,1,selected.type === "partners" ? 24 : 12)});}}/></label>}
